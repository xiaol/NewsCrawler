...
class Parser(object):
    ...
    def get_title(cls, root):
        attrs = [{'attr': 'class', 'value': 'alpha tweet-title'}]
    ...
    def get_date(cls, root):
        attrs = [{'attr': 'class', 'value': 'timestamp'},
                 ]
    ...
    def get_content(cls, url, root):
        ...
        attrs = [{'attr': 'class', 'value': 'article-entry text'},
                 ]
    ...
    def cont_format(node):
        ...
        for child in node.iter():
            ...
            # tags filter
            if 'class' in child.attrib and child.attrib['class'] == 'article-entry text':
                continue
            if 'class' in child.attrib and child.attrib['class'] == 'tag':
                break

            # video frame
            if 'id' in child.attrib and child.attrib['id'] == 'social-after-wrapper':
                continue
