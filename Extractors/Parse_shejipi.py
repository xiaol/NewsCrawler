...
class Parser(object):
    ...
    def get_title(cls, root):
        attrs = [{'attr': 'class', 'value': 'post-title heading-font'}]
    ...
    def get_date(cls, root):
        attrs = [{'attr': 'class', 'value': 'post-date-comments'},
                 ]
    ...
    def get_content(cls, url, root):
        ...
        attrs = [
            {'attr': 'class', 'value': 'post-page-content'},
            ]
    ...
    def cont_format(node):
        ...
        try:
            node.find('.//div[@class="wp-polls"]').drop_tree()
        ...
        for child in node.iter():
            ...
            # tags filter
            if 'id' in child.attrib and child.attrib['id'] == 'post-page-content':
                continue
            if 'class' in child.attrib and child.attrib['class'] == 'wp-polls':
                break
            if 'class' in child.attrib and child.attrib['class'] == 'wp-polls-loading':
                break
            if 'id' in child.attrib and child.attrib['id'] == 'pr':
                break
            ...
            # news content
            if child.tag == 'p' or child.tag == 'h2':
