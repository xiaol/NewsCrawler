...
class Parser(object):
    ...
    def get_title(cls, root):
        attrs = [{'attr': 'name', 'value': 'entry-title'}]
    ...
    def get_date(cls, root):
        attrs = [{'attr': 'class', 'value': 'entry-date updated'},
                 ]
    ...
    def get_content(cls, url, root):
        ...
        attrs = [
            # {'attr': 'class', 'value': 'td-paragraph-padding-1'},
            {'attr': 'class', 'value': 'td-post-text-content'},
            ]
    ...
    def cont_format(node):
        ...
        try:
            node.find('.//div[@class="essb_links  essb_counters essb_displayed_both essb_template_round-retina'
                      ' essb_959191419 print-no "]').drop_tree()
        ...
        for child in node.iter():
            ...
            # tags filter
            if 'class' in child.attrib and child.attrib['class'] == 'td-paragraph-padding-1':
                continue
            if 'class' in child.attrib and child.attrib['class'] == 'line-share':
                break
