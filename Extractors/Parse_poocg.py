...
class Parser(object):
    ...
    def get_list(cls, root, xp):
        urls = root.xpath(xp)
        urls = [i.text_content().strip() for i in urls]
        return urls or []

    @classmethod
    def get_title(cls, root):
        # attrs = [{'attr': 'class', 'value': 'zptit'}]
        # tag = Extractor.get_ment_by_attrs(root, attrs)
        # tag = root.xpath('.//h1[@class="zptit"]')
        # if tag is not None:
        ...
        attrs = [{'attr': 'class', 'value': 'viewCon'},
                 ]
        conts = Extractor.get_ment_by_attrs(root, attrs)
        if conts:
            for child in conts.iter():
    ...
    def get_author(cls, root):
        attrs = [{'attr': 'class', 'value': 'name'}]
        origin = Extractor.get_ment_by_attrs(root, attrs)
        if origin is not None:
            origin = origin.text_content().strip()
            return origin
        else:
            return None
    ...
    def cont_format(node):
        ...
        try:
            node.find('.//div[@class="titarea"]').drop_tree()
